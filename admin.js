...
class AdminPanel {
    ...
    saveTracksToDB(tracks) {
        ...
        const saveNext = (index) => {
            ...
            reader.onload = (e) => {
                const trackData = {
                    name: track.name,
                    fileName: track.fileName,  // 保存原始文件名
    ...
    loadPlaylist() {
        ...
        request.onsuccess = () => {
            ...
            this.playlist = savedTracks.map(track => {
                ...
                return {
                    name: track.name,
                    fileName: track.fileName || track.name,  // 保存原始文件名
                    file: file,
                    url: URL.createObjectURL(file),
                    duration: track.duration,
                    textData: track.textData
    ...
    exportToJSON() {
        ...
        // 转换播放列表为 JSON 格式
        const jsonData = this.playlist.map((track) => {
            // 使用原始文件名
            const audioFileName = track.fileName || `${track.name}.mp3`;
            // URL 编码文件名（处理中文、空格等特殊字符）
            const encodedFileName = encodeURIComponent(audioFileName);

            return {
                name: track.name,
                url: `https://raw.githubusercontent.com/tikzrt-bot/player/main/${encodedFileName}`,
        ...
        const cleanedData = jsonData.map(item => {
            ...
            if (cleanItem.duration === null) delete cleanItem.duration;
            if (cleanItem.textData === null) delete cleanItem.textData;
            return cleanItem;
        });
        ...
        // 显示提示
        alert('✅ playlist.json 已导出！\n\n接下来请：\n1. 将音频文件上传到 tikzrt-bot/player 仓库（保持原文件名）\n2. 将 playlist.json 复制到 tikzrt-bot.github.io/player/ 目录\n3. 提交两个仓库的更改');
    }
