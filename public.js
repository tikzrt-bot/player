...
class PublicPlayer {
    ...
    initDB() {
        ...
        // 先尝试从 JSON 文件加载播放列表
        this.loadPlaylistFromJSON().then(data => {
            if (data && data.length > 0) {
                console.log('从 JSON 文件加载到', data.length, '个曲目');
                this.playlist = data;
                // 按照 day1/day2/day3 顺序排序
                this.sortPlaylist();
                ...
                // 如果 JSON 加载失败或为空，使用 IndexedDB
                console.log('JSON 文件为空或加载失败，使用 IndexedDB');
                this.initIndexedDB();
            }
        }).catch(error => {
            console.log('JSON 加载失败，使用 IndexedDB:', error);
            this.initIndexedDB();
        });
    }
